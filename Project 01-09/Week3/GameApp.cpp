...
void GameApp::UpdateScene(float dt) //dt为两帧间隔时间
{
    ...
    static float cubePhi = 0.0f, cubeTheta = 0.0f, cubeScale = 1.0f; //记录旋转角度、缩放倍率
    static float trans_cubePhi = 0.0f, trans_cubeTheta = 0.0f, trans_cubeDelta = 0.0f; //记录平移距离
    ...
    if (ImGui::Begin("Use ImGui"))//创建一个名为"Use ImGui"的窗口
    {
        ...
        {
            //旋转角度、缩放倍率、平移距离初始化
            cubePhi = cubeTheta = 0.0f;
            cubeScale = 1.0f;
            trans_cubePhi = trans_cubeTheta = trans_cubeDelta = 0.0f;
        }
        ImGui::SliderFloat("Scale", &cubeScale, 0.2f, 10.0f);//用一个滑条来控制缩放，并显示当前缩放倍率
        ImGui::SliderFloat("Phi", &cubePhi, -XM_PI, XM_PI);     // 显示转动角度
        ImGui::SliderFloat("Theta", &cubeTheta, -XM_PI, XM_PI);  // 显示转动角度
    }
    //关闭窗口
    ImGui::End();
    ...
    if (!ImGui::IsAnyItemActive())//当ImGui的UI处于非活跃状态时
    {
        ...
        {
            trans_cubePhi += io.MouseDelta.x * 0.01f;
            trans_cubeTheta -= io.MouseDelta.y * 0.01f;
        }
        if (ImGui::IsMouseDragging(ImGuiMouseButton_Left))//拖动左键旋转
        {
            cubePhi -= io.MouseDelta.y * 0.01f;
            cubeTheta -= io.MouseDelta.x * 0.01f;
        }
        if (io.MouseWheel!=0.0f)
        {
            cubeScale += io.MouseWheel * 0.2f;
            if (cubeScale > 10.0f)//设置缩放速率阈值
                cubeScale = 10.0f;
            else if (cubeScale < 0.2f)
                cubeScale = 0.2f;
        }
    }
    /******************************************************************************************************/
    //更新常量缓冲区
    //先缩放，再旋转，最后平移
    m_CBuffer.world = XMMatrixTranspose(XMMatrixScaling(cubeScale, cubeScale, cubeScale)*
        XMMatrixRotationY(cubeTheta) * XMMatrixRotationX(cubePhi) *
        XMMatrixTranslation(trans_cubePhi, trans_cubeTheta, trans_cubeDelta));
